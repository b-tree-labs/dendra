...
Optional integrations (see :mod:`dendra.cloud.verdict_telemetry`) can
...
The ``DENDRA_NO_TELEMETRY`` environment variable short-circuits the
...
_NO_TELEMETRY_ENV = "DENDRA_NO_TELEMETRY"
...
def register_default_emitter(factory: Callable[[], TelemetryEmitter]) -> None:
    ...
    The factory is called once per :class:`~dendra.core.LearnedSwitch`
...
def get_default_emitter() -> TelemetryEmitter:
    ...
    Honors ``$DENDRA_NO_TELEMETRY``: when set to a truthy value, returns
...
def _telemetry_disabled_by_env() -> bool:
    """Return True iff ``$DENDRA_NO_TELEMETRY`` is set to a truthy value.
