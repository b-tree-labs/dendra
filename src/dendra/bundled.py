...
- ``DENDRA_BUNDLED_CDN_BASE`` — replace the default CDN base URL
  with a local/private mirror (testing, air-gapped sites,
  enterprise proxy).
- ``DENDRA_BUNDLED_CACHE_DIR`` — override the cache location.
- ``DENDRA_BUNDLED_OFFLINE=1`` — skip the network entirely;
...
def ensure_model(role: Role, *, progress: bool = True) -> Path:
    ...
    Honours ``DENDRA_BUNDLED_OFFLINE=1`` — when set, raises
