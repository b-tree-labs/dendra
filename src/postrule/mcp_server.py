...
"""Model Context Protocol (MCP) server for Postrule.

Exposes Postrule's CLI surface as MCP tools so Claude Code (and other
MCP-aware agents) can drive Postrule directly inside a user's codebase
without shelling out.

v1 ships exactly four tools:

- ``postrule_analyze`` (path, format) -> dict
- ``postrule_init`` (file, function_name, dry_run=True) -> dict
- ``postrule_refresh`` (path, check_only=False) -> dict
- ``postrule_doctor`` (path) -> dict

All tools return JSON-serializable dicts. ``postrule_init`` defaults to
``dry_run=True`` so the agent never writes files via MCP unless the
caller is explicit. The handlers call into Postrule's existing modules
in-process (no subprocess hop).

Run as a stdio server:

    python -m postrule.mcp_server
...
# Fail-import gracefully if the user installed postrule without the mcp extra.
try:
    from mcp import types
    from mcp.server import Server
except ImportError as e:  # pragma: no cover - exercised manually
    raise ImportError(
        "The Postrule MCP server requires the optional 'mcp' dependency. "
        "Install it with: pip install postrule[mcp]"
    ) from e
...
_TOOL_DEFINITIONS: list[types.Tool] = [
    types.Tool(
        name="postrule_analyze",
        description=(
            "Run the Postrule static analyzer over a file or directory. "
            "Identifies classification sites (return-string dispatch, dict "
            "lookups, regex routers, model prompts) and returns a "
            "structured report identical to `postrule analyze --json`."
        ),
        inputSchema={
            "type": "object",
            "properties": {
        ...
        inputSchema={
            ...
            "properties": {
                ...
                "labels": {
                    ...
                    "items": {"type": "string"},
                    "description": (
                        "Explicit labels. If omitted, Postrule infers them "
    ...
    types.Tool(
        name="postrule_refresh",
        description=(
            "Walk the project for __postrule_generated__/ files and report "
    ...
    types.Tool(
        name="postrule_doctor",
        description=(
            "Diagnostic-only walk of __postrule_generated__/ files. Same "
            "counts as postrule_refresh, plus per-detail severity tags "
...
def _tool_analyze(args: Mapping[str, Any]) -> dict[str, Any]:
    from postrule.analyzer import analyze, render_json
    ...
    if fmt != "json":
        from postrule.analyzer import render_markdown, render_text
...
def _walk_drift(root: Path) -> list[dict[str, Any]]:
    """Walk ``root`` for __postrule_generated__/ files, return per-file rows.
    ...
    for gen_path in root.rglob("__postrule_generated__/*.py"):
        try:
        ...
        if not candidate.exists():
            ...
            matches = [p for p in root.rglob(short) if "__postrule_generated__" not in p.parts]
...
_SUGGESTIONS = {
    ...
    "source_drift": "run `postrule refresh` to regenerate",
    "user_edited": "run `postrule refresh --force` to overwrite manual edits, or revert them",
    "missing": "run `postrule refresh` to regenerate the missing file",
    "orphaned": (
        "delete the generated file; the source function is gone (or run `postrule refresh --prune`)"
    ),
    "malformed": "delete the generated file and re-run `postrule init` for the source function",
}
        ...
        annotated.append(
            {
                **r,
                ...
                "suggestion": _SUGGESTIONS.get(status, "run `postrule refresh` for guidance"),
            }
        )
...
_HANDLERS = {
    "postrule_analyze": _tool_analyze,
    "postrule_init": _tool_init,
    "postrule_refresh": _tool_refresh,
    "postrule_doctor": _tool_doctor,
}
...
def build_server() -> Server:
    ...
    server: Server = Server("postrule", version=_postrule_version())
...
def main() -> int:
    """Entry point for ``python -m postrule.mcp_server``."""
    # Sanity guard: the server is silent on stdout (stdio is reserved for
    # the MCP transport). Diagnostic output goes to stderr.
    if os.environ.get("POSTRULE_MCP_DEBUG"):
        import sys

        print("Postrule MCP server starting on stdio", file=sys.stderr)
    serve_stdio()
    return 0
