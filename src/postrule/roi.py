...
Reads outcome logs produced by :class:`postrule.storage.FileStorage` and
...
class ROIAssumptions:
    ...
    # With Postrule: first site is ~0.5 weeks, subsequent sites ~0.1 weeks.
    postrule_first_site_weeks: float = 0.5
    postrule_subsequent_site_weeks: float = 0.1
    ...
    # Postrule-graduated, what fraction of outcomes would have gone to the
    # language model? Default: 100% (Postrule's rule/ML paths save all of them).
    pct_outcomes_that_would_use_llm_without_postrule: float = 1.0
...
class SwitchROI:
    ...
    # Token savings — model calls Postrule routed away from.
...
def compute_switch_roi(
    *,
    ...
    # Direct engineering savings: (baseline - postrule) weeks × cost.
    # For a SINGLE site already instrumented in Postrule, the savings are
    # "baseline - postrule-subsequent" (assumed amortized across sites).
    eng_low = (
        a.baseline_weeks_per_graduation_low - a.postrule_subsequent_site_weeks
    ) * a.engineer_cost_per_week_usd
    eng_high = (
        a.baseline_weeks_per_graduation_high - a.postrule_subsequent_site_weeks
    ) * a.engineer_cost_per_week_usd
    ...
    # more chances for a silent regression that Postrule's breaker catches.
    ...
    # Token-cost savings: every outcome that Postrule routed through
    ...
    counterfactual_llm_calls = (
        model_calls_avoided * a.pct_outcomes_that_would_use_llm_without_postrule
    )
    cost_per_call_low = (
...
def format_portfolio_report(
    ...
    lines = [
        "Postrule self-measured ROI report",
        "=" * 60,
    ...
    lines += [
        ...
        f"  postrule_subsequent_site={a.postrule_subsequent_site_weeks}w",
        ...
        f"  counter_factual_llm_pct={a.pct_outcomes_that_would_use_llm_without_postrule:.0%}",
        "",
