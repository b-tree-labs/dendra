...
"""Auto-generate pre-registered hypothesis files at `postrule init` time.

Every wrapped switch gets a markdown file at
``postrule/hypotheses/<switch>.md`` populated from cohort-tuned
...
edits; second `postrule init` runs print a notice and skip.
"""
...
# Mirror BAKED_IN_DEFAULTS in postrule.insights.tuned_defaults; we don't
...
def hypothesis_path(switch_name: str, root: Path | str = "postrule/hypotheses") -> Path:
...
def generate_hypothesis_file(
    ...
    *,
    ...
    root: Path | str = "postrule/hypotheses",
...
def _render_template(
    *,
    ...
    lines.append("## Verdict (filled in by `postrule report`)")
    lines.append("")
    lines.append(
        "*Once the gate fires (or the timeout at outcome "
        f"{pred_high * 3} triggers a 'did not graduate within "
        "budget' outcome), `postrule report` populates this section "
    ...
    lines.append(
        ...
        "Auto-generated by `postrule init`; review and commit before "
