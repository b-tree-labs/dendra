...
from postrule.cloud.report.aggregator import HypothesisVerdict, SwitchMetrics
...
def render_switch_card(
    ...
    *,
    ...
    if transition_chart_path:
        ...
        parts.append("> *Chart rendering pending — install `postrule[viz]` to generate the PNG.*\n")
    ...
    if cost_per_call is not None:
        ...
        parts.append(
            ...
            f"`postrule report {metrics.switch_name} --model claude-haiku-4.5` to "
    ...
    parts.append("")
    parts.append(
        f"*Regenerate with `postrule report {metrics.switch_name}`. "
        f"Last drift check: "
...
def _render_hypothesis_table(verdict: HypothesisVerdict, *, switch_name: str) -> str:
    ...
    rows = [
        ...
        f"[`postrule/hypotheses/{switch_name}.md`](../hypotheses/{switch_name}.md) "
