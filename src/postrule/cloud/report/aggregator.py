...
:class:`~postrule.core.ClassificationRecord` log, bucket into
...
from postrule.gates import GateDecision, McNemarGate
...
class HypothesisVerdict:
    ...
    from ``postrule/hypotheses/<switch>.md``). Fields are ``None`` when
...
def aggregate_switch(
    ...
    *,
    ...
    storage
        ...
        a :class:`~postrule.storage.Storage` instance, but duck-typed.
    switch_name
    ...
    alpha
        ...
        matches the default :class:`~postrule.gates.McNemarGate` config.
    min_paired
...
def _is_correct(record: ClassificationRecord, prediction: Any) -> bool:
    ...
    Outcome semantics (matching :data:`~postrule.verdicts.Verdict`):
    ...
    See ``postrule/gates.py:_paired_correctness`` for the matching
