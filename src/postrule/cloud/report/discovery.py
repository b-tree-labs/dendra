...
When a customer first runs ``postrule analyze --report``, this module
...
anything. Drives the conversion funnel from `postrule analyze` to
`postrule init`.
...
class OpportunitySite:
    ...
    Built from the analyzer's :class:`~postrule.analyzer.ClassificationSite`
...
def render_discovery_report(
    ...
    *,
    ...
    ----------
    analyze_report
        An :class:`~postrule.analyzer.AnalyzerReport` instance from a
        recent ``postrule.analyzer.analyze(path)`` call.
    cost_per_call
    ...
    # ---- Header ----------------------------------------------------------
    parts.append("# Initial Analysis — what `postrule analyze` found in this codebase\n")
    parts.append(
    ...
    if not sites and not already:
        parts.append(
            ...
            "don't yet recognize — `postrule analyze --verbose` prints the "
            "shapes considered.\n"
        )
    else:
        first_action = ""
    ...
    parts.append(
        "After `postrule init` on the first site, this discovery report "
        "is replaced by `postrule/results/_summary.md` (the cockpit view "
        "across all wrapped switches). Each wrapped switch gets its own "
        "report card at `postrule/results/<switch>.md` that fills in over "
        "time:\n\n"
...
def _opportunities_table(sites: list[OpportunitySite], cost_per_call: float) -> str:
    ...
    for i, s in enumerate(sites[:15], start=1):  # cap top-15 for readability
        ...
        if s.lift_status == "auto_liftable":
            action = f"`postrule init {s.file_path}:{s.function_name}`"
        else:
    ...
    rows.append(
        ...
        f"Switch with `postrule report --pro-forma --model claude-haiku-4.5` to see "
