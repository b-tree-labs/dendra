...
The ``postrule report --summary`` view answers: "How is my whole
...
from postrule.cloud.report.aggregator import SwitchMetrics, aggregate_switch
...
def render_project_summary(
    ...
    *,
    ...
    parts.append("")
    parts.append(
        "*Regenerate with `postrule report --summary`. Per-switch deep-dives "
        "live alongside this file in `postrule/results/`.*"
    )
    parts.append("")
...
def _cockpit_blockquote(summary: ProjectSummary) -> str:
    ...
    if n == 0:
        return (
            ...
            "> Run `postrule analyze --report` to discover candidate sites, "
            "then `postrule init <file>:<func>` on each to wrap."
        )
...
def _hypothesis_rollup(summary: ProjectSummary) -> str:
    ...
    postrule/hypotheses/): compares observed vs predicted intervals.
    """
