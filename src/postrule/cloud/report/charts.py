...
report module works without the ``postrule[viz]`` extra installed.
Callers should catch :class:`ImportError` and fall back to the
text-only "Chart rendering pending — install postrule[viz]" placeholder.
...
from postrule.cloud.report.aggregator import SwitchMetrics
...
def _apply_brand_style(plt) -> None:
    """Configure matplotlib once per render with Postrule's brand palette."""
    plt.rcParams.update(
        {
...
def transition_curve(
    ...
    *,
    ...
    if ``postrule[viz]`` is not installed.
