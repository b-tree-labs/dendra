...
:meth:`postrule.core.LearnedSwitch.record_verdict` to
``POST /v1/verdicts`` on the hosted API. Default-on for signed-in
users (those with ``~/.postrule/credentials``); inert for everyone else.
...
- **Honors opt-outs at every level.** Process: ``POSTRULE_NO_TELEMETRY=1``
  short-circuits at default-emitter resolution (see
  :mod:`postrule.telemetry`). Per-switch: ``telemetry=NullEmitter()`` on
...
``postrule.auth.is_logged_in()`` returns True. The trigger is the
import from :mod:`postrule` — :func:`maybe_install` is invoked from
``postrule/__init__.py`` so simple ``import postrule`` is enough.
...
#: Default hosted-API base URL. Override with ``$POSTRULE_API_URL``.
DEFAULT_API_URL: Final[str] = "https://api.postrule.ai"
...
# doesn't bloat ``pip install postrule``.
# ---------------------------------------------------------------------------
...
class CloudVerdictEmitter:
    ...
    Construct directly for explicit control (tests, dev backends):
        ...
        em = CloudVerdictEmitter(
            ...
            bearer_token="prul_live_…",  # pragma: allowlist secret
        )
...
# ---------------------------------------------------------------------------
# One-shot install hook called from postrule/__init__.py.
# ---------------------------------------------------------------------------
...
def maybe_install(
    *,
    ...
    The single ``POSTRULE_API_URL`` env var overrides the hosted endpoint
    (used by the local-dev Worker on ``http://localhost:8787``); falls
    back to :data:`DEFAULT_API_URL` in production.
    """
    if os.environ.get("POSTRULE_NO_TELEMETRY", "").strip().lower() not in (
        "",
        "0",
        "false",
        "no",
    ...
    if creds.get("telemetry_enabled") is False:
        ...
        # toggle. The flag is cached locally in ~/.postrule/credentials at
        # `postrule login` time (see ``cli.cmd_login``) and refreshed
        # opportunistically when ``GET /v1/whoami`` disagrees with the
        # local cache. Treat as if the env var were set.
        return None

    resolved_url = (api_url or os.environ.get("POSTRULE_API_URL") or DEFAULT_API_URL).rstrip("/")
...
def uninstall() -> None:
    ...
    from postrule.telemetry import reset_default_emitter
