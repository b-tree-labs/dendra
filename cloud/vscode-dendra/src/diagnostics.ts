/**
 * DendraDiagnosticsProvider walks open Python documents, asks the dendra
...
import { CliRunner, ClassificationSite, DendraCliError, Hazard, AnalyzerReport } from "./dendraClient";

export const DIAGNOSTIC_SOURCE = "dendra";
...
/**
 * DendraDiagnosticsProvider — owns a DiagnosticCollection, runs the CLI on
 * Python documents, publishes diagnostics. Degrades gracefully if the CLI
 * is missing.
 */
export class DendraDiagnosticsProvider {
  ...
  async refresh(doc: vscode.TextDocument): Promise<void> {
    ...
    try {
      ...
      if (e instanceof DendraCliError && e.code === "not_found") {
        this.warnCliMissing();
        this.collection.delete(doc.uri);
        return;
      }
      ...
      console.warn(`[dendra] analyze failed for ${filePath}: ${(e as Error).message}`);
      this.collection.delete(doc.uri);
      return;
    }
  ...
  private warnCliMissing(): void {
    ...
    void vscode.window.showWarningMessage(
      "Dendra CLI not found on PATH; run `pip install dendra` or set dendra.binaryPath.",
    );
  }
}
