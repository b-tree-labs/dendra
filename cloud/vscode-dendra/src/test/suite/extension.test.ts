 ...
 *
 * Note: this test does NOT shell out to the real dendra CLI. We construct
 * the analyzer report ourselves and feed it through DendraDiagnosticsProvider
...
import { DendraDiagnosticsProvider, DIAGNOSTIC_SOURCE } from "../../diagnostics";
import { AnalyzerReport, CliRunner } from "../../dendraClient";
...
suite("Extension integration", () => {
  test("publishes a diagnostic on the refused fixture", async () => {
    ...
    const provider = new DendraDiagnosticsProvider(new StaticCli(report));
  ...
  test("dendra.rescanWorkspace command is registered", async () => {
    const all = await vscode.commands.getCommands(true);
    assert.ok(all.includes("dendra.rescanWorkspace"), "dendra.rescanWorkspace should be registered");
    assert.ok(all.includes("dendra.runAutoLift"), "dendra.runAutoLift should be registered");
  });
});
