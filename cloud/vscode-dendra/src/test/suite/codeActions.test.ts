 ...
 *
 * Coverage:
 *   - A function with hazards yields exactly one "Dendra: run auto-lift here"
 *     CodeAction.
 *   - A clean function yields zero CodeActions.
 *   - The CodeAction's command targets dendra.runAutoLift with the right args.
 */

import * as assert from "assert";
import * as vscode from "vscode";
import { DendraDiagnosticsProvider } from "../../diagnostics";
import { DendraCodeActionProvider, RUN_AUTO_LIFT_COMMAND, RunAutoLiftArgs } from "../../codeActions";
...
suite("DendraCodeActionProvider", () => {
  test("yields one auto-lift action for a function with hazards", async () => {
    ...
    const provider = new DendraDiagnosticsProvider(new FakeCli(report));
    await provider.refresh(doc);
    const actions = new DendraCodeActionProvider(provider).provideCodeActions(
      doc,
      new vscode.Range(0, 0, 4, 0),
    ) as vscode.CodeAction[];
    assert.strictEqual(actions.length, 1);
    assert.strictEqual(actions[0].title, "Dendra: run auto-lift here");
